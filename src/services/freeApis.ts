...
// Free, no API key, no hard limits (respect 1 req/sec)
export async function searchLocations(query: string): Promise<LocationResult[]> {
  ...
  const res = await fetch(url, {
    headers: { 'User-Agent': 'TimeTravel-StreetView-App/1.0' }
  });
...
// Free, no API key, no limits
export function getPollinationsImageUrl(prompt: string, width: number, height: number): string {
  const safePrompt = prompt.replace(/Text overlay:.*?$/im, '').trim().substring(0, 500);
  const encoded = encodeURIComponent(safePrompt + ', documentary photography, highly detailed, photorealistic');
  return `https://image.pollinations.ai/prompt/${encoded}?width=${width}&height=${height}&nologo=true&seed=${Date.now()}`;
}

export async function generateFreeImage(prompt: string, aspectRatio: string): Promise<string> {
  const dims = getImageDimensions(aspectRatio);
  const url = getPollinationsImageUrl(prompt, dims.width, dims.height);
  const res = await fetch(url);
...
export async function generateFreePrompts(params: {
  ...
  const systemPrompt = `You are a prompt engineer for AI image generation. Generate exactly ${numImages} image prompts for a time-travel sequence. Return ONLY a valid JSON array with objects having "year" (integer) and "prompt" (string) fields. No markdown, no explanation, just the JSON array.`;
...
Rules:
1. Same camera angle in every prompt
2. Core buildings and layout identical
